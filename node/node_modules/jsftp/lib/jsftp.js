...
var COMMANDS = [
    ...
    "abor", "pwd", "cdup", "feat", "noop", "quit", "pasv", "syst", "help",
    // Commands with one or more parameters
    "cwd", "dele", "list", "mdtm", "mkd", "mode", "nlst", "pass", "retr", "rmd",
    "rnfr", "rnto", "site", "stat", "stor", "type", "user", "xrmd", "opts", "mlsd",
    // Extended features
    "chmod", "size"
];
...
var Ftp = module.exports = function(cfg) {
    ...
    this.useList = this.useList || false;
...
Ftp.prototype._createStreams = function(socket) {
    ...
    this.pipeline.on("error", function() {
        self.reemit("error");
    });
};

Ftp.prototype.parseResponse = function(data) {
    if (data.isError && !this.cmdBuffer_.length) {
        if (data.code === 421) {
            this.emit("customError", data);
        } else {
            console.log("[jsftp] Error: ", data);
        }
    }
            ...
            socket.on("close", function(err) {
                var t = setInterval(function() {
					if (listing === "") {
						err = {
							"code": 450,
							"text": "450 "+path+": No such file or directory",
							"isMark": false,
							"isError": true
						};
					}
