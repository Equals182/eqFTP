...
var Ftp = function (cfg) {
    ...
    this.foldreRetrievingMethod = this.foldreRetrievingMethod || "LIST";
...
Ftp.prototype.auth = function(params) {
    ...
    self.raw({
        ...
        callback: function(err, data) {
            console.log('[eqFTP-loginTest]', err, data);
            if (err || [230, 331, 332].indexOf(data.code) == -1) {
                ...
                console.log('[eqFTP-loginTest] sending pass');
                self.raw({
                    command: "pass",
                    ...
                    callback: function(err, data) {
                        ...
                        if (err || [202, 230].indexOf(data.code) == -1) {
                            ...
                            self.raw({
                                command: "feat",
                                callback: function(err, data) {
                                    console.log("[eqFTP-speciallyForWikunia] FEAT callback start", err, data);
                                    if (err) {
                                        console.log("[eqFTP-speciallyForWikunia] There's error. Calling HELP. ");
                                        self.raw({
                                            command: "help",
                                            callback: function(err, data) {
                                                if (err) {
                                        ...
                                        console.log("[eqFTP-speciallyForWikunia] No error. Checking params.callback", params.callback);
                                        if (params.callback) {
                                            console.log("[eqFTP-speciallyForWikunia] Calling params.callback");
                                            params.callback(false, data);
                                        }
                                    }
                                }
...
Ftp.prototype.raw = function(params) {
    /*
        ...
        arguments
    */
    console.log('[eqFTP-rawTest] start');
    if (!params.command) {
        console.log('[eqFTP-rawTest] no command', params);
        if (params.callback) {
            params.callback();
        }
    }

    console.log('[eqFTP-rawTest] step 2');
    var commandString = params.command 
    if (params.arguments && params.arguments.length > 0) {
        commandString = commandString + " " + params.arguments.join(" ");
    }
    console.log('[eqFTP-rawTest] Command string: ', commandString.replace(/^pass\s.*/g, "PASSWORD HIDDEN :P"));

    console.log('[eqFTP-rawTest] Pushing to queue: ', {command: commandString.replace(/^pass\s.*/g, "PASSWORD HIDDEN :P"), callback: params.callback, callback100: params.callback100});
    this.commandQueue.push({command: commandString, callback: params.callback, callback100: params.callback100});
    console.log('[eqFTP-rawTest] Queue is: ', this.commandQueue);
    if (!this.inProgress) {
        console.log('[eqFTP-rawTest] doing queue');
        this.doQueue();
    }
};
Ftp.prototype.doQueue = function() {
    if (this.commandQueue.length > 0) {
        ...
        console.log('[eqFTP-rawTest] doQueue. Command is: ', command.replace(/^pass\s.*/g, "PASSWORD HIDDEN :P"));

        if (command && typeof command == "string") {
            this.emit("cmdSend", command);
            this.pipeline.write(command + "\r\n");
            console.log('[eqFTP-rawTest] doQueue. command is probably sent. Fuck there\'s a lot to debug...');

            if (this.debugMode) {
                this.emit('jsftp_debug', 'user_command', command.replace(/^pass\s.*/g, "PASSWORD HIDDEN :P") || {});
            }
        }
    }
};
Ftp.prototype.parseResponse = function(data) {
    ...
    console.log('[eqFTP-rawTest] parsing response', data);
    if (data.code < 200) {
        if (this.commandQueue[0].callback100) {
            console.log('[eqFTP-rawTest] callback100', this.commandQueue[0].callback100);
            this.commandQueue[0].callback100(data.isError, data);
        }
    } else {
        if (this.commandQueue[0]) {
            console.log('[eqFTP-rawTest] no callback', this.commandQueue[0]);
            if (this.commandQueue[0].callback) {
                console.log('[eqFTP-rawTest] calling', this.commandQueue[0].callback);
                this.commandQueue[0].callback(data.isError, data);
            }
            this.commandQueue.shift();
        } else {
            console.log('[eqFTP-rawTest] emitting customError', data);
...
Ftp.prototype.ls = function(path, callback) {
    var self = this,
        command = this.foldreRetrievingMethod || "LIST",
        noExist = false,
        bigError = false;
    var commandToSend = {
        command: command,
        ...
        callback: function(err, data) {
            if (err && ( data.code == 550 || data.code == 450) ) {
                noExist = data;
                if (self.foldreRetrievingMethod === "MLSD") {
    ...
    this.sendCommand({
        ...
        onClose: function(err) {
            if (!bigError && !noExist) {
                ...
                if (command === "LIST") {
                    if (self.debugMode) {
                        self.emit('jsftp_debug', 'eqFTP-LISTING', listing);
                    }
                    if (!bigError && listing != "") {
                    ...
                    l.forEach(function(o, n) {
                        var row = o.split(" "),
                            params = row.splice(0, 1),
                            name = row.join(" ").replace('\r',''),
                            h_params = params[0].split(";"),
                            respParams = {};
                        if (name && params) {
...
Ftp.prototype.useCommand = function(command) {
    this.foldreRetrievingMethod = command;
}
...
Ftp.prototype.setDebugMode = function(debug) {
