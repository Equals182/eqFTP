 ...
 * @param {Number}   [options.passTimeout=1000]    Password delay timeout in milliseconds. Default: 1000ms
...
(function() {
    ...
    this.connect = function(cbconnect) {
        ...
        var connectionTimeout = setTimeout(function() {
            ...
            if (cbconnect)
                cbconnect({message: "SSH connection timeout"});
        }, 30000);
        ...
        this.socket.on("end", function(code) {
            if (cbconnect) {
                cbconnect({message: "SSH Process Died", lines: debugLines});
            }
    ...
    this.disconnect = function(cbdisconn) {
        cbdisconn = cbdisconn || function(){}; //noOp
        if (this.state & Sftp.STATE_DISCONNECTED)
            return cbdisconn();
        var _self = this;
    ...
    this.readFile = function(filename, encoding, callback) {
        ...
        if (!this.$readCache)
            this.$readCache = {};
        var cache = this.$readCache[filename],
            now   = Date.now(),
            _self = this;
        if (cache) {
            if (cache.expires >= now)
                ...
                delete this.$readCache[filename];
        }
